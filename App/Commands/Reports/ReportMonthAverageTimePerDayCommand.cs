...
internal class ReportMonthAverageTimePerDayCommand : Command
{
    ...
    private void ProjectSummaryHandler(int monthOffset)
    {
        ...
        var records = _dbRepository.Records.GetByMonth(monthStart).Where(r => r.RE_StartedAt < DateTime.Now.DayBegin());

        if (records is not null)
        {
            var workingDays = records.Select(r => r.RE_StartedAt.Date).Distinct().Count();
            var expectedHours = Convert.ToDecimal(workingDays * 7.5);
            var totalHours = records.Sum(r => r.TimeSpentHours);
            var overtimeHours = totalHours - expectedHours;
            ...
            _console.MarkupLineInterpolated($"Overtime hours: [{Colors.Primary}]{(overtimeHours >= 0 ? overtimeHours : ""):N2} hours[/]");
